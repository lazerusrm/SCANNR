...
use egui::{Color32, Pos2, Rect, Stroke, Vec2 as EguiVec2, Shape};
use glam::Vec2;
...
impl LODLevel {
    pub fn from_zoom_and_count(zoom: f32, node_count: usize) -> Self {
        ...
        if node_count > 1000 {
            if effective_zoom < 0.5 { LODLevel::Minimal }
            else if effective_zoom < 1.0 { LODLevel::Low }
            else if effective_zoom < 2.0 { LODLevel::Medium }
            else { LODLevel::High }
        } else if node_count > 100 {
            if effective_zoom < 0.3 { LODLevel::Low }
...
impl TopologyRenderer {
    ...
    fn draw_grid(&self, painter: &egui::Painter, viewport_rect: Rect, view_state: &TopologyViewState) {
        let grid_spacing = 50.0 * view_state.zoom;
        if grid_spacing < 5.0 { return; } // Don't draw if too dense
        ...
        while x < viewport_rect.width() {
            painter.line_segment(
                [Pos2::new(viewport_rect.min.x + x, viewport_rect.min.y), Pos2::new(viewport_rect.min.x + x, viewport_rect.max.y)],
                stroke,
            );
            x += grid_spacing;
        }
        ...
        while y < viewport_rect.height() {
            painter.line_segment(
                [Pos2::new(viewport_rect.min.x, viewport_rect.min.y + y), Pos2::new(viewport_rect.max.x, viewport_rect.min.y + y)],
                stroke,
            );
            y += grid_spacing;
        }
    ...
    fn render_edges(&self, painter: &egui::Painter, viewport_rect: Rect, view_state: &TopologyViewState) {
        for edge_idx in self.graph.graph.edge_indices() {
            ...
            let stroke = Stroke::new(self.config.edge_width * view_state.zoom.sqrt(), self.config.edge_color);
            
            // Draw curved line (quadratic bezier)
            let mid = screen_u.lerp(screen_v, 0.5);
            let dist = screen_u.distance(screen_v);
            let normal = EguiVec2::new(screen_v.y - screen_u.y, screen_u.x - screen_v.x).normalized();
    ...
    fn render_nodes(&self, ui: &egui::Ui, painter: &egui::Painter, viewport_rect: Rect, view_state: &TopologyViewState) {
        ...
        for node_idx in self.graph.graph.node_indices() {
            ...
            let stroke_color = if is_selected { self.config.selected_node_color } 
                              else if is_hovered { self.config.hovered_node_color } 
                              else { self.config.node_stroke_color };
            ...
            if lod.show_labels() {
                ...
                let galley = ui.fonts(|f| f.layout_no_wrap(label.clone(), egui::FontId::proportional(font_size), Color32::WHITE));
                let text_rect = Rect::from_center_size(screen_pos + EguiVec2::new(0.0, radius + font_size), galley.size());
