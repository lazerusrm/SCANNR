...
impl DeviceClassification {
    ...
    pub fn is_likely_gateway(&self, ip: Ipv4Addr, hostname: Option<&str>, ports: &[u16]) -> bool {
        ...
        if let Some(host) = hostname {
            ...
            if host_lower.contains("gateway")
                || host_lower.contains("router")
                || host_lower.contains("firewall")
            {
                return true;
            }
        }

        let gateway_ports = [22, 23, 80, 443, 53];
        let port_count = ports.iter().filter(|p| gateway_ports.contains(p)).count();
        if port_count >= 3 {
            return true;
        }
